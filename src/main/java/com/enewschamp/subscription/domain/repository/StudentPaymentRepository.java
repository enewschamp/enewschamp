...
public interface StudentPaymentRepository extends JpaRepository<StudentPayment, Long> {
	@Query("Select s from StudentPayment s where s.studentId = :studentId and s.editionId= :editionId")
	public Optional<StudentPayment> getByStudentIdAndEdition(@Param("studentId") Long studentId,
