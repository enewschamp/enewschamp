package com.enewschamp.app.admin.publication.champs.repository;
...
@Repository
public class StudentChampsRepositoryCustomImpl extends RepositoryImpl
		implements IGenericListRepository<Champ>, AdminConstant {

	@PersistenceContext
	private EntityManager entityManager;

	@Override
	public Page<Champ> findAll(Pageable pageable, AdminSearchRequest searchRequest) {
		CriteriaBuilder cb = entityManager.getCriteriaBuilder();
		CriteriaQuery<Champ> criteriaQuery = cb
				.createQuery(Champ.class);
		Root<Champ> monthlySummaryRoot = criteriaQuery
				.from(Champ.class);
		...
		// Build query
		TypedQuery<Champ> q = entityManager.createQuery(criteriaQuery);
		if (pageable.getPageSize() > 0) {
			int pageNumber = pageable.getPageNumber();
			q.setFirstResult(pageNumber * pageable.getPageSize());
			q.setMaxResults(pageable.getPageSize());
		}
		List<Champ> list = q.getResultList();
