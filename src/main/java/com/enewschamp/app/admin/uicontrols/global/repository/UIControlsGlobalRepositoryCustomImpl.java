...
public class UIControlsGlobalRepositoryCustomImpl extends RepositoryImpl implements IGenericListRepository<UIControlsGlobal> {
	...
	public Page<UIControlsGlobal> findAll(Pageable pageable, AdminSearchRequest searchRequest) {
		...
		// Build query
		TypedQuery<UIControlsGlobal> q = entityManager.createQuery(criteriaQuery);
		if (pageable.getPageSize() > 0) {
			int pageNumber = pageable.getPageNumber();
			q.setFirstResult(pageNumber * pageable.getPageSize());
			q.setMaxResults(pageable.getPageSize());
		}
		List<UIControlsGlobal> list = q.getResultList();
		long count = getRecordCount(criteriaQuery, filterPredicates, uiControlsGlobalRoot);
