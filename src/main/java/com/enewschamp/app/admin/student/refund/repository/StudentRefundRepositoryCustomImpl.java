package com.enewschamp.app.admin.student.refund.repository;
...
public class StudentRefundRepositoryCustomImpl extends RepositoryImpl
	...
	@Override
	public Page<StudentRefund> findAll(Pageable pageable, AdminSearchRequest searchRequest) {
		...
		// Build query
		TypedQuery<StudentRefund> q = entityManager.createQuery(criteriaQuery);
		if (pageable.getPageSize() > 0) {
			int pageNumber = pageable.getPageNumber();
			q.setFirstResult(pageNumber * pageable.getPageSize());
			q.setMaxResults(pageable.getPageSize());
		}
		List<StudentRefund> list = q.getResultList();
		long count = getRecordCount(criteriaQuery, filterPredicates, studentRefundRoot);
