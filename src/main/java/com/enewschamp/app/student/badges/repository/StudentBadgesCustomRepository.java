...
public interface StudentBadgesCustomRepository extends JpaRepository<RecognitionData, Long> {
	...
	@Query("select new RecognitionData(d.studentBadgesId as studentBadgesId,b.editionId as editionId,b.readingLevel as readingLevel,d.badgeId as badgeId,b.nameId as badgeName,d.operationDateTime as badgeGrantDate,b.genreId as badgeGenre,d.yearMonth as yearMonth)  from StudentBadges d,Badge b where d.badgeId=b.badgeId and d.studentId = :studentId and d.yearMonth>= :yearMonth and d.recordInUse ='Y' order by d.operationDateTime desc")
	public List<RecognitionData> getStudentBadgeDetails(@Param("studentId") Long studentId,
			@Param("yearMonth") Long yearMonth);

	@Query("select d.badgeId as badgeId from StudentBadges d where d.badgeId=:badgeId and d.studentId = :studentId and d.yearMonth= :yearMonth and d.recordInUse ='Y' order by d.operationDateTime desc")
	public Long isBadgeUnlocked(@Param("badgeId") Long badgeId, @Param("studentId") Long studentId,
			@Param("yearMonth") Long yearMonth);
