	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < listOfFiles.length; i++) {
		      if (listOfFiles[i].isFile() && !listOfFiles[i].getName().startsWith(".")) {
		        ...
		        tempPNG = resizeImage(img, Double.valueOf(newDimension.getWidth()).intValue(),  Double.valueOf(newDimension.getHeight()).intValue());
		        tempJPG = resizeImage(img, Double.valueOf(newDimension.getWidth()).intValue(),  Double.valueOf(newDimension.getHeight()).intValue());
...
//	        System.out.println(img.getHeight() + " X " + img.getWidth());
			...
			if (imageType.equals(JPG_IMAGE_TYPE)) {
				...
				ImageIO.write(tempJPG, PNG_IMAGE_TYPE, newFileJPG);
			}
