...
public class AuditBuilderOld {
	...
	private AuditBuilderOld(AuditService auditService, ObjectMapper objectMapper,
	...
	public static AuditBuilderOld getInstance(AuditService auditService, ObjectMapper objectMapper,
			EnewschampApplicationProperties appConfig) {
		return new AuditBuilderOld(auditService, objectMapper, appConfig);
	}

	public AuditBuilderOld forParentObject(Object parentObjectInstance) {
		this.parentObject = parentObjectInstance;
		deriveCollectionProperties(this.parentObject.getClass());
		return this;
	}

	public AuditBuilderOld forChildObject(Object chidObjectInstance) {
...
//				});
				...
				collectionChanges.forEach(collectionChange -> {
					if (collectionChange instanceof ValueAdded) {
						...
						handleItemAddition(commitMetadata, auditDTO, listChange.getPropertyName(),
								((InstanceId) itemAdded.getValue()));
					}
					if (collectionChange instanceof ValueRemoved) {
					...
					if (collectionChange instanceof ElementValueChange) {
						handleChildElementValueChange(commitMetadata, auditDTO, listChange.getPropertyName(),
	...
	private void getChildAuditForSameCommitId(CommitMetadata commitMetadata, AuditDTO auditDTO,
			boolean isForNewObject) {
					...
					byChildObject.get().forEach(change -> {
						if (change instanceof ValueChange && !isForNewObject) {
							handleItemModification(commitMetadata, auditDTO,
									...
									(InstanceId) change.getAffectedGlobalId(), (ValueChange) change);
						}
	...
	private void buildChildChangeAuditDTO(CommitMetadata commitMetadata, AuditDTO childAuditDTO,
			ChangesByObject byObject, String typeName) {
		byObject.get().forEach(change -> {
			if (change instanceof ValueChange) {
